...
filtered_df = df[(df["Sex"].isin(gender_filter)) & (df["Smoker"].isin(smoker_filter))]
...
st.header("OBJECTIVE 1: TO COMPARE INSURANCE CHARGES BETWEEN SMOKERS AND NON-SMOKERS ACCORDING TO AGE CATEGORY")

charges_by_age_smoker = (filtered_df.groupby(["Age Group", "Smoker"])["Charges"].mean().reset_index())

fig1, ax1 = plt.subplots(figsize=(10, 6))

sns.barplot(data=charges_by_age_smoker,x="Age Group",y="Charges",hue="Smoker",palette={"yes": "red", "no": "blue"},ax=ax1)

ax1.set_title("Average Insurance Charges by Age Group and Smoker Status",fontsize=14,fontweight="bold")
...
st.header("OBJECTIVE 2: TO ANALYZE THE RELATIONSHIP BETWEEN INSURANCE CHARGES AND BMI")
